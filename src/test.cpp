...
int run_r1cs_h_se_ppzksnark_setup(size_t num_constraints, size_t input_size, std::string outputDir) {
    ...
    std::chrono::duration<double> elapsed = end - start;
    logFile << "================================================================================\n";
    logFile << "ALgorithm: R1CS H-SE-ppzkSNARK Generator\n";
    logFile << "================================================================================\n";
...
int run_setup(std::string algorithm, size_t num_constraints, size_t input_size, std::string outputDir)
{
    if (algorithm == "groth16") {
        return run_r1cs_gg_ppzksnark_setup<default_r1cs_gg_ppzksnark_pp>(num_constraints, input_size, outputDir);
    }else if (algorithm == "gm17") {
        printf("暂未实现");
        return -1;
    }else if (algorithm == "hse") {
        return run_r1cs_h_se_ppzksnark_setup<default_r1cs_gg_ppzksnark_pp>(num_constraints, input_size, outputDir);
    }else {
        printf("未知算法:%s", algorithm);
        return -1;
    }
}

int run_prove(std::string algorithm, size_t num_constraints, size_t input_size, std::string outputDir) 
{
    if (algorithm == "groth16") {
        return run_r1cs_gg_ppzksnark_prove<default_r1cs_gg_ppzksnark_pp>(num_constraints, input_size, outputDir);
    }else if (algorithm == "gm17") {
        printf("暂未实现");
        return -1;
    }else if (algorithm == "hse") {
        return run_r1cs_h_se_ppzksnark_prove<default_r1cs_gg_ppzksnark_pp>(num_constraints, input_size, outputDir);
    }else {
        printf("未知算法:%s", algorithm);
        return -1;
    }
...
int run_verify(std::string algorithm, size_t num_constraints, size_t input_size, std::string outputDir)
{
    if (algorithm == "groth16") {
        return run_r1cs_gg_ppzksnark_verify<default_r1cs_gg_ppzksnark_pp>(num_constraints, input_size, outputDir);
    }else if (algorithm == "gm17") {
        printf("暂未实现");
        return -1;
    }else if (algorithm == "hse") {
        return run_r1cs_h_se_ppzksnark_verify<default_r1cs_gg_ppzksnark_pp>(num_constraints, input_size, outputDir);
    }else {
        printf("未知算法:%s", algorithm);
        return -1;
    }
...
int test_groth16() {
    ...
    cout << "Verification status: " << (verified ? "PASS": "FAIL") << endl;
...
int test_hse() {
    ...
    // generate keypair
    const r1cs_h_se_ppzksnark_keypair<default_r1cs_gg_ppzksnark_pp> keypair = r1cs_h_se_ppzksnark_generator<default_r1cs_gg_ppzksnark_pp>(constraint_system);
    ...
    // generate proof
    const r1cs_h_se_ppzksnark_proof<default_r1cs_gg_ppzksnark_pp> proof = r1cs_h_se_ppzksnark_prover<default_r1cs_gg_ppzksnark_pp>(keypair.pk, pb.primary_input(), pb.auxiliary_input());

    // verify
    bool verified = r1cs_h_se_ppzksnark_verifier_strong_IC<default_r1cs_gg_ppzksnark_pp>(keypair.vk, pb.primary_input(), proof);
...
int main (int argc, char* argv[]) {
    ...
    if (scheme == "groth16") {
        ...
        std::cerr << "Invalid scheme. Available schemes: groth16, gm17, hse" << std::endl;
        return 1;
    }

    // if (argc < 6) {
    //     std::cerr << "Usage: " << argv[0] << "<groth16|gm17|hse> <setup|prove|verify> num_constraints input_size outputDir" << std::endl;
    //     return 1;
    // }
    // std::string algorithm = argv[1];
    // std::string mode = argv[2];
    // size_t num_constraints = std::stoul(argv[3]);
    ...
    // if (mode == "setup") {
    //     run_setup(algorithm, num_constraints, input_size, outputDir);
    // } else if (mode == "prove") {
    //     run_prove(algorithm, num_constraints, input_size, outputDir);
    // } else if (mode == "verify") {
    //     run_verify(algorithm, num_constraints, input_size, outputDir);
    // } else {
